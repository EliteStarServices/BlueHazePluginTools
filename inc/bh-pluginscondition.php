...
class PluginsCondition {
	...
	public function __construct() {
		...
		//register_activation_hook( plugin_dir_path( __DIR__ ) . 'bh-plugin-tools.php', array( $this, 'cron_start' ) );
		//register_deactivation_hook( plugin_dir_path( __DIR__ ) . 'bh-plugin-tools.php', array( $this, 'cron_stop' ) );
		...
		//register_activation_hook( plugin_dir_path( __DIR__ ) . 'bh-plugin-tools.php', array( $this, 'notify_cron_start' ) );
		//register_deactivation_hook( plugin_dir_path( __DIR__ ) . 'bh-plugin-tools.php', array( $this, 'notify_cron_stop' ) );

	}


	/** ==================================================
	 * View Notification Plugins
	 *
	...
	public function installed_plugins( $links, $file ) {
		...
		update_option( 'plg_cond_update_date_time', 'Plugin ' . __( 'Alerts' ) . ' : ' . $current );
	...
	public function plugins_condition_wp_cron() {
		...
		update_option( 'plg_cond_update_date_time', 'Plugins ' . __( 'Updated' ) . ' : ' . $current );
	...
	public function plugins_condition_notify_wp_cron() {
		...
		$content   = '<blockquote>' . sprintf( __( 'This email is delivered every %1$s days to the administrator by Plugins Condition.', 'bh-plugin-tools' ), get_option( 'plg_cond_notify_interval', 30 ) ) . '</blockquote>';
	...
	private function main_func( $file, $installed_plugin ) {
		...
		if ( ! empty( $slug ) ) {
			if ( $installed_plugin && get_transient( 'plg_cond_datas_' . $slug . '_' . get_locale() ) ) {
				...
				if ( is_wp_error( $call_api ) ) {
					...
					/* Set cache ~ 12 Hours*/
					set_transient( 'plg_cond_datas_' . $slug . '_' . get_locale(), $call_apis, 43200 );
				}
			}
		}
		...
		if ( ! empty( $call_apis ) ) {
			...
			if ( $pg_ver === $wp_ver ) {
				...
				// COMMENT BELOW DISABLES VERSION WARNING
			...
			if ( 1 > $time_lag_date ) {
				$color_date = 'green';
				$pg_date    = __( 'Under 24 Hours', 'bh-plugin-tools' );
			...
			$html_ver = '<span style="color: indigo;">' . __( '3rd Party / Unofficial', 'bh-plugin-tools' ) . '</span>';
			$caution .= $html_ver;
		}
	...
	public function dashboard_text() {
		...
		if ( 'dashboard' === $screen->id ) {
			...
			<b>
			<?php
			echo wp_kses_post( sprintf( __( 'See %s For Extended Plugin Information.', 'bh-plugin-tools' ), $installed_plugin_html ) );
			if ($network_plugin_url) {
				echo "<br>".wp_kses_post( sprintf( __( 'Visit %s Administration Page.', 'bh-plugin-tools' ), $network_plugin_html ) );
			}
			?>
			</b><hr>
			<?php
